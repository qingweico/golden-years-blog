...
public class ArticleDetailRestApi extends BaseRestApi {
    ...
    public Result incPagViews(@RequestParam String articleId) {
        ...
        redisTemplate.setnx60s(RedisConf.REDIS_ARTICLE_ALREADY_READ + SysConf.SYMBOL_COLON + articleId + SysConf.SYMBOL_COLON + visitIp, visitIp);
        redisTemplate.increment(RedisConf.REDIS_ARTICLE_READ_COUNTS + SysConf.SYMBOL_COLON + articleId, 1);
        return Result.ok();
    }
    ...
    public Result praiseArticleById(@RequestBody IdBO idBO) {
        ...
        // 设置用户点赞标志位(已点赞)
        redisTemplate.set(RedisConf.REDIS_ARTICLE_ALREADY_STAR +
                SysConf.SYMBOL_COLON + articleId + SysConf.SYMBOL_HYPHEN + userId, "true");
        // 点赞数增加
        redisTemplate.increment(RedisConf.REDIS_ARTICLE_STAR_COUNTS + SysConf.SYMBOL_COLON + articleId, 1);
    ...
    public Result cancelStar(@RequestBody IdBO idBO) {
        ...
        String userId = idBO.getUserId();
        // 设置用户点赞标志位(未点赞)
        redisTemplate.del(RedisConf.REDIS_ARTICLE_ALREADY_STAR +
                SysConf.SYMBOL_COLON + articleId + SysConf.SYMBOL_HYPHEN + userId);
        // 点赞数减少
        redisTemplate.decrement(RedisConf.REDIS_ARTICLE_STAR_COUNTS + SysConf.SYMBOL_COLON + articleId, 1);
    ...
    public Result getArticleStarCounts(@RequestParam String articleId) {
        String articleStarCounts = redisTemplate.get(RedisConf.REDIS_ARTICLE_STAR_COUNTS + SysConf.SYMBOL_COLON + articleId);
    ...
    public Result isStar(@RequestBody IdBO idBO) {
        String articleId = idBO.getArticleId();
        String userId = idBO.getUserId();
        String isStar = redisTemplate.get(RedisConf.REDIS_ARTICLE_ALREADY_STAR +
                SysConf.SYMBOL_COLON + articleId + SysConf.SYMBOL_HYPHEN + userId);
    ...
    public Result collectArticleById(@RequestBody CollectBO collectBO) {
        ...
        // 设置用户收藏标志位(已收藏)
        redisTemplate.set(RedisConf.REDIS_ARTICLE_ALREADY_COLLECT +
                SysConf.SYMBOL_COLON + articleId +
                SysConf.SYMBOL_HYPHEN + userId +
                SysConf.SYMBOL_HYPHEN + favoritesId, "true");
        // 文章收藏数增加
        redisTemplate.increment(RedisConf.REDIS_ARTICLE_COLLECT_COUNTS + SysConf.SYMBOL_COLON + articleId, 1);
        // 用户收藏夹收藏数增加
        redisTemplate.increment(RedisConf.REDIS_ARTICLE_COLLECT_COUNTS +
    ...
    public Result isCollect(@RequestBody CollectBO collectBO) {
        ...
        // 获取Redis中特定前缀
        Set<String> keys = redisTemplate.keys(RedisConf.REDIS_ARTICLE_ALREADY_COLLECT +
                SysConf.SYMBOL_COLON + articleId +
                SysConf.SYMBOL_HYPHEN + userId +
    ...
    public Result getArticleCollectCounts(@RequestParam String articleId) {
        String articleCollectCounts = redisTemplate.get(RedisConf.REDIS_ARTICLE_COLLECT_COUNTS + SysConf.SYMBOL_COLON + articleId);
    ...
    public Result cancelCollect(@RequestBody CollectBO collectBO) {
        ...
        if (cancelCollect) {
            ...
            // 用户收藏夹收藏数减少
            redisTemplate.decrement(RedisConf.REDIS_ARTICLE_COLLECT_COUNTS +
                    SysConf.SYMBOL_COLON + userId +
                    SysConf.SYMBOL_HYPHEN + favoritesId, 1);
    ...
    public Result getFavorites(@RequestParam String userId, @RequestParam String articleId) {
        ...
        for (Favorites favorite : favoriteList) {
            ...
            // 精确匹配favoritesId
            String isFavoritesCollect = redisTemplate.get(RedisConf.REDIS_ARTICLE_ALREADY_COLLECT +
                    SysConf.SYMBOL_COLON + articleId +
                    SysConf.SYMBOL_HYPHEN + userId +
                    SysConf.SYMBOL_HYPHEN + favoritesId);
            // 判断用户的收藏夹是否收藏了该文章
