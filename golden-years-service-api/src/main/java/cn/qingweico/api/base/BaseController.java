    ...
    public <T> T getLoginUser(Class<T> loginUserType, String tokenKey, String infoKey) {
        ...
        try {
            ...
            if (claims != null) {
                ...
                t = JsonUtils.jsonToPojo(redisTemplate.get(infoKey + SysConst.SYMBOL_COLON + id), loginUserType);
            }
        } catch (Exception e) {
            GraceException.error(Response.TICKET_INVALID);
        }
        if (!StringUtils.isEmpty(id)) {
            String redisToken = redisTemplate.get(tokenKey + SysConst.SYMBOL_COLON + id);
